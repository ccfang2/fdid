...
#' @description The \code{cov_intrpl} function is used to perform natural spline interpolation on the covariance matrix.
...
#'
#' @return The \code{cov_intrpl} function returns a numeric matrix of covariance interpolation.
#' @export
#'
...
#' grid <- seq(0,1,len=5)
#' cov_intrpl_mat <- cov_intrpl(cov=cov_mat, grid=grid, n_intrpl=50)
cov_intrpl <- function(cov, grid, n_intrpl) {
  ...
  cov_intrpl <- matrix(Phi_fine %*% coef_spline, nrow = n_intrpl, ncol = n_intrpl)
