...
fdid_scb <- function(object=NULL,
  ...
  if(is.null(df)){
    if (any(is.na(covhat_spline_pre))) {
      scb_pre <- ffscb::confidence_band_fragm(x=betahat_spline_pre, diag.cov.x=diag_covhat_spline_pre, tau=hat.tau_pre, type = "FFSCB.z", conf.level=1-(1-conf.level)/2, n_int=1)
    } else {
      scb_pre <- ffscb::confidence_band(x=betahat_spline_pre, cov.x=covhat_spline_pre, tau=hat.tau_pre, type="FFSCB.z", conf.level=1-(1-conf.level)/2, n_int=1)
    }
  } else{
    if (any(is.na(covhat_spline_pre))) {
      scb_pre <- ffscb::confidence_band_fragm(x=betahat_spline_pre, diag.cov.x=diag_covhat_spline_pre, tau=hat.tau_pre, df=df, type = "FFSCB.t", conf.level=1-(1-conf.level)/2, n_int=1)
    } else {
      scb_pre <- ffscb::confidence_band(x=betahat_spline_pre, cov.x=covhat_spline_pre, tau=hat.tau_pre,  df=df, type="FFSCB.t", conf.level=1-(1-conf.level)/2, n_int=1)
    }
  }
  ...
  if(is.null(df)){
    if (any(is.na(covhat_spline_post))) {
      scb_post <- ffscb::confidence_band_fragm(x=betahat_spline_post, diag.cov.x=diag_covhat_spline_post, tau=hat.tau_post, type = "FFSCB.z", conf.level=1-(1-conf.level)/2, n_int=1)
    } else {
      scb_post <- ffscb::confidence_band(x=betahat_spline_post, cov.x=covhat_spline_post, tau=hat.tau_post, type="FFSCB.z", conf.level=1-(1-conf.level)/2, n_int=1)
    }
  } else{
    if (any(is.na(covhat_spline_post))) {
      scb_post <- ffscb::confidence_band_fragm(x=betahat_spline_post, diag.cov.x=diag_covhat_spline_post, tau=hat.tau_post, df=df, type = "FFSCB.t", conf.level=1-(1-conf.level)/2, n_int=1)
    } else {
      scb_post <- ffscb::confidence_band(x=betahat_spline_post, cov.x=covhat_spline_post, tau=hat.tau_post,  df=df, type="FFSCB.t", conf.level=1-(1-conf.level)/2, n_int=1)
    }
  }
