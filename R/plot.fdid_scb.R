...
#' @param ta.s a numeric vector of control parameters with length two for deriving the honest reference band under the violation of no-anticipation assumption. If NULL, the critical value \eqn{t_{1-\alpha/2, df}} or \eqn{z_{1-\alpha/2}} is applied with the same \eqn{\alpha} used for constructing the point-wise confidence intervals. If no treatment anticipation is defined in \code{ta.ts}, the value of \code{ta.s} is ignored.
#' @param frmtr.m a numeric vector of control parameters with length two for deriving the honest reference band under violation of the parallel trends assumption. The default is NULL.
...
plot.fdid_scb <- function(object,
  ...
  if (ta.ts==t0 & is.null(frmtr.m)) {
    ...
    if (isTRUE(note.post)) {
      ...
      # sig_intervals <- rbind(sig_pos_intervals, sig_neg_intervals)
      # if(!all(is.na(sig_intervals))) sig_intervals <- na.omit(sig_intervals)
      # sig_intervals <- cbind(sig_intervals, sig_intervals[,2]-sig_intervals[,1])
      # sig_intervals <- sig_intervals[order(sig_intervals[, 3], decreasing = TRUE),]
      #
      # if(is.matrix(sig_intervals)) {
      #   sig_intervals <- sig_intervals[,-3]
      # } else {
      #   sig_intervals <- matrix(sig_intervals,nrow=1)
      # }

      par(xpd = TRUE) # Turn off clipping so we can draw *outside* the plot
      segments(x_mid,  y_top,  x_mid,  y_top + box_height, col = "black")

      # if(!all(is.na(sig_intervals))) {
      #   sig_intervals <- round(sig_intervals, 1)
      #   intervals <- sprintf("[%.1f, %.1f]", sig_intervals[,1], sig_intervals[,2])
      #   if(length(intervals)>1) {
      #     intervals_text <- paste(intervals[1], collapse = "")
      #     text((x_mid + x_right)/2, y_top + 0.5*box_height, paste("Testing:", intervals_text, "..."), cex = scale.legend)
      #   } else {
      #     intervals_text <- paste(intervals, collapse = "")
      #     text((x_mid + x_right)/2, y_top + 0.5*box_height, paste("Testing:", intervals_text), cex = scale.legend)
      #   }
      # } else {
    ...
    if (isTRUE(note.pre) & !isTRUE(note.post)) {
      ...
      box_height <- 0.1 * (y_top - y_bottom)  # height = 10% of plot range
      x_mid      <- ta.ts # border between boxes

      par(xpd = TRUE) # Turn off clipping so we can draw *outside* the plot
      segments(x_mid,  y_top,  x_mid,  y_top + box_height, col = "black")

      # x_vals_bf_ta.ts <- seq(start, ta.ts, len=100)
      # if ( all(honest_ub_splinefun(x_vals_bf_ta.ts) > scb_ub_splinefun(x_vals_bf_ta.ts)) &  all(honest_lb_splinefun(x_vals_bf_ta.ts) < scb_lb_splinefun(x_vals_bf_ta.ts)) ) {
      #   text((x_left + x_mid)/2, y_top + 0.5*box_height, "Validating: Pass", cex = scale.legend)
      # } else {
      #   text((x_left + x_mid)/2, y_top + 0.5*box_height, "Validating: Fail", cex = scale.legend)
      # }
      text((x_left + x_mid)/2, y_top + 0.5*box_height, "Validating", cex = scale.legend)
      par(xpd = FALSE) # Turn clipping back on
    }
    ...
    if (isTRUE(note.pre) & isTRUE(note.post)) {
      ...
      box_height <- 0.1 * (y_top - y_bottom)  # height = 10% of plot range
      x_mid_pre  <- ta.ts
      x_mid_post <- t0      # border between boxes
      ...
      # x_vals_bf_ta.ts <- seq(start, ta.ts, len=100)
      # if ( all(honest_ub_splinefun(x_vals_bf_ta.ts) > scb_ub_splinefun(x_vals_bf_ta.ts)) & all( honest_lb_splinefun(x_vals_bf_ta.ts) < scb_lb_splinefun(x_vals_bf_ta.ts)) ) {
      #   text((x_left + x_mid_pre)/2, y_top + 0.5*box_height, "Validating: Pass", cex = scale.legend)
      # } else {
      #   text((x_left + x_mid_pre)/2, y_top + 0.5*box_height, "Validating: Fail", cex = scale.legend)
      # }
      ...
      #   if(length(intervals)>1) {
      #     intervals_text <- paste(intervals[1], collapse = "")
      #     text((x_mid_post + x_right)/2, y_top + 0.5*box_height, paste("Testing:", intervals_text, "..."), cex = scale.legend)
      #   } else {
      #     intervals_text <- paste(intervals, collapse = "")
      #     text((x_mid_post + x_right)/2, y_top + 0.5*box_height, paste("Testing:", intervals_text), cex = scale.legend)
      #   }
